...
`thefreshoffice.omalexia` is an Omarchy shell plugin (bar widget + panel) for
[Omalexia](../README.md). Installed by `install.sh`, which copies
this directory to `~/.config/omarchy/plugins/thefreshoffice.omalexia/` and enables
...
IPC: `omarchy-shell thefreshoffice.omalexia open|close|toggle|read|stop|dictate|status`.
